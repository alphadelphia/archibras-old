...
#else
    const char* pszModule = "harmonian";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Harmonian ";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "harmonian.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "harmoniand.pid"));
